...
export const PostCard: React.FC<PostCardProps> = ({
    ...
    const mainImage = imagePreviewUrl ? (
        <Image src={imagePreviewUrl} alt="Post image" fill className="object-contain z-10" sizes="20vw" />
    ) : (
        <div className="w-full h-full bg-slate-200" />
    );
    const hashtagElement = hashtags.length > 0 && (
        <p className="text-xl opacity-90 mt-auto flex-shrink-0 pt-4">{hashtags.join(' ')}</p>
    );
            ...
            case 'footer-focus':
                return (
                    <div className="w-full h-full flex flex-col bg-white">
                        ...
                        <div className="flex-shrink-0 p-6 text-slate-800 border-t-4" style={{ borderColor: brandColor }}>
                ...
                return (
                    <div className="w-full h-full flex flex-col bg-white">
                        <div className="w-full aspect-square relative flex-shrink-0">
                            {imageBackground}{mainImage}{logoElement}
                        </div>
