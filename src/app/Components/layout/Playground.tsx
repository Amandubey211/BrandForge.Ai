...
import { Copy, Download, Sparkles, UploadCloud, FileCheck2, X, Move, Bot, Wand2, Info, ChevronDown, Eye, Square, Columns } from 'lucide-react';
import { LayoutTemplate, PostCard } from '../Ui/PostCard';
...
export const Playground: React.FC<PlaygroundProps> = ({
  ...
  const canvasAspectRatio = activeTemplate === 'default' ? '4/5' : '16/9';

  return (
    <>
      <main className="min-h-screen bg-slate-50 bg-grid px-20">
        <div className="flex h-screen">
          {/* --- LEFT CONTROL PANEL (30%) --- */}
          <div className="w-full max-w-md bg-white border-r border-slate-200 shadow-xl flex flex-col h-screen rounded-2xl">
            ...
            <div className="p-4 border-t border-slate-200 space-y-4 flex-shrink-0">
               ...
               <AnimatePresence>
                 {generatedPost && (<motion.div className="flex items-center justify-center gap-4" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}><button onClick={handleCopy} className="flex items-center gap-2 rounded-md px-4 py-2 text-sm font-medium text-slate-700 bg-slate-100 hover:bg-slate-200 transition-colors w-1/2 justify-center"><Copy className="h-4 w-4" /> Copy Text</button><button onClick={handleDownload} className="flex items-center gap-2 rounded-md px-4 py-2 text-sm font-medium text-white transition-colors w-1/2 justify-center" style={{ backgroundColor: brandColor }}><Download className="h-4 w-4" /> Download</button></motion.div>)}
               </AnimatePresence>
          ...
          {/* --- RIGHT CANVAS (70%) --- */}
          <div ref={dragConstraintsRef} className="flex-grow flex items-center justify-center p-8  relative h-screen">
            <motion.div
              layout
              transition={{ type: 'spring', stiffness: 300, damping: 30 }}
              className="w-full max-w-xl rounded-lg shadow-2xl overflow-hidden relative"
                ...
                {generatedPost && (
                  <>
                    <motion.div
                      ...
                      className="absolute top-0 left-0 cursor-grab active:cursor-grabbing p-2 z-40"
                      style={{ x: headlinePosition.x, y: headlinePosition.y }}
                    >
                      <div
                        contentEditable
                        suppressContentEditableWarning
                        onBlur={e => setHeadlineContent(e.currentTarget.innerHTML)}
                    ...
                    {logoBase64 && (
                      <motion.div
                        ...
                        className="absolute top-0 left-0 cursor-grab active:cursor-grabbing group z-40"
                      ...
                      >
                        <div className="relative w-16 h-16 p-1 bg-white/50 rounded-lg backdrop-blur-sm">
                          ...
                          <Image src={logoBase64} alt="Logo" fill className="object-contain" />
                        </div>
                      </motion.div>
                    )}
                  </>
